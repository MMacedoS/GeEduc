  ...
  <div class="card mb-3">
    <div class="card-body">
      <div class="m-0">
        ...
        <input type="number" step="0000" min="2019" readonly name="academic_year" class="form-control" value="<?= $turma_disciplina->ano_letivo ?? ($school_year ?? date('Y')) ?>">
      </div>
    </div>
  </div>
</div>
    ...
    <div class="card-body">
      <div class="m-0">
        ...
        <select name="active" class="form-control" id="">
          <option value="0" <?php if (isset($turma_disciplina->ativo)  && $turma_disciplina->ativo == '0') {
                              echo 'selected';
                            } ?>>Impedido</option>
          <option value="1" selected <?php if (isset($turma_disciplina->ativo)  && $turma_disciplina->ativo == '1') {
                                        echo 'selected';
                                      } ?>>Disponivel</option>
        </select>
      </div>
  ...
  <div class="card mb-3">
    <div class="card-body">
      <div class="d-flex flex-wrap gap-2 justify-content-end">
        ...
        <? $yearParam = isset($school_year) ? '?school_year=' . $school_year : ''; ?>
        <a href="/turmas/<?= $turma->uuid ?>/disciplinas/<?= $yearParam ?>" class="btn btn-secondary">Cancelar</a>
      </div>
    </div>
  </div>
</div>